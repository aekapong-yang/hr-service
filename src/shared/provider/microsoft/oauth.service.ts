...
export class OAuthService {
  ...
  async getAccessToken(code: string): Promise<TokenInfoResponse> {
    ...
    const { data } = await firstValueFrom(
      this.httpService
        .post<TokenInfoResponse>(`${process.env.MS_URL}/token`, form)
        .pipe(
  ...
  async getUserProfile(token: string) {
    ...
    const { data } = await firstValueFrom(
      this.httpService
        .get<UserProfileResponse>("https://graph.microsoft.com/v1.0/me", { headers })
        .pipe(
          catchError((err: AxiosError) => {
